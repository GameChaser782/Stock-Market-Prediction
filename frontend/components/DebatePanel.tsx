...
import { Search, TrendingUp, TrendingDown, Scale, Loader2, AlertCircle } from "lucide-react";
  ...
  return (
    <div className="flex flex-col h-full p-4 gap-4">
      ...
      <div className="flex gap-2">
        <div className="flex-1 flex items-center gap-2 bg-white/5 border border-white/10 rounded-xl px-4 py-3 focus-within:border-violet-500 transition-colors">
        ...
        <select
          ...
          className="bg-white/5 border border-white/10 rounded-xl px-3 text-sm text-white outline-none focus:border-violet-500"
        >
          {[1, 2, 3, 4, 5].map((r) => (
            <option key={r} value={r} className="bg-[#1a1a2e]">{r}R</option>
          ))}
        </select>
        <button
          onClick={startDebate}
          disabled={!ticker.trim() || loading}
          className="flex items-center gap-2 bg-violet-600 hover:bg-violet-500 disabled:opacity-40 disabled:cursor-not-allowed text-white px-5 py-3 rounded-xl text-sm font-medium transition-colors"
        >
      ...
      {loading && (
        <div className="flex-1 flex flex-col items-center justify-center gap-4 text-gray-400">
          ...
          <p className="text-xs text-gray-600">Fetching data, building arguments, judging...</p>
        </div>
      )}
        ...
        <div className="flex-1 overflow-y-auto space-y-4">
          ...
          <div className="bg-gradient-to-br from-violet-900/40 to-indigo-900/40 border border-violet-500/30 rounded-2xl p-5">
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
              <div className="bg-white/5 rounded-xl p-3">
                ...
                <div className="h-1.5 bg-white/10 rounded-full mt-2">
                  <div className="h-full bg-violet-500 rounded-full" style={{ width: `${result.verdict?.confidence}%` }} />
                </div>
              </div>
              <div className="bg-white/5 rounded-xl p-3">
                <div className="text-xs text-gray-400 mb-1">Stronger Case</div>
                <div className={`font-semibold capitalize ${result.verdict?.stronger_case === "bull" ? "text-green-400" : result.verdict?.stronger_case === "bear" ? "text-red-400" : "text-yellow-400"}`}>
            ...
            {result.verdict?.recommendation && (
              <p className="text-sm text-gray-300 border-t border-white/10 pt-3">{result.verdict.recommendation}</p>
            )}
          </div>
          ...
          <div className="flex gap-1">
            {result.bull_arguments.map((_, i) => (
              <button
                ...
                className={`flex-1 py-1.5 rounded-lg text-xs font-medium transition-colors ${activeRound === i ? "bg-violet-600 text-white" : "bg-white/5 text-gray-400 hover:bg-white/10"}`}
              >
                Round {i + 1}
              </button>
            ))}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-green-900/20 border border-green-500/20 rounded-xl p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-sm font-medium text-green-400">Bull Case</span>
              </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-white/5 border border-white/10 rounded-xl p-4">
              <div className="text-xs text-green-400 mb-2">Bull Summary</div>
          ...
          <p className="text-xs text-gray-600 text-center">{result.verdict?.disclaimer}</p>
        </div>
      )}
